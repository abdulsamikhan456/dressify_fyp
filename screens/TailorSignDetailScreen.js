...
const ModalView = ({ visible, onClose, onNext, navigation }) => {
  ...
  const handleNext = async () => {
    if (shopName.trim() === "" || shopAddress.trim() === "" || selectedCity === null || selectedArea.trim() === "") {
      Alert.alert("Error", "Shop Name, Shop Address, City, and Area are required");
    } else {
      try {
        if (uid && userRole) {
          // Add tailor details to Firestore
          const tailorDocRef = await addDoc(collection(db, "users", uid, "tailorDetails"), {
            shopName,
            shopAddress,
            city: selectedCity,
            area: selectedArea,
          ...
          console.log("Tailor details added with ID: ", tailorDocRef.id);
        ...
        console.error("Error adding tailor details: ", error.message);
      }
    }
  };
    ...
    <Modal animationType="slide" transparent={true} visible={visible} onRequestClose={handleClose}>
      <View style={styles.modalContainer}>
        <View style={styles.modalContent}>
          <Text style={styles.modalTitle}>Enter Shop Details</Text>
